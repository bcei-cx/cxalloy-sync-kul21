...
CHECKLIST_PAGE = r'''
  ...
  <section class="page" id="page-checklists">
    ...
    <div class="grid">
      <div class="card c12">
        <div class="card-head">
          <div><h3>Checklist completion by equipment type</h3><div class="sub">Weighted checklist completion for each equipment type, split by commissioning level</div></div>
        </div>
...
function clRenderTypeChart(){
  ...
  const levels=checklistState.level==='ALL'?LEVELS:LEVELS.filter(l=>l.k===checklistState.level);
  const datasets=levels.map(l=>({label:l.k,data:types.map(t=>{const rs=checklistState.filtered.filter(r=>txtValue(r.equipment_type)===t);return +clLevelStats(rs,l.k).p.toFixed(1);}),backgroundColor:cssVar(l.varc),borderColor:l.k==='L5'?cssVar('--line'):'transparent',borderWidth:l.k==='L5'?1:0}));
  const box=document.getElementById('clTypeChartBox');box.style.height=Math.max(330,types.length*30+90)+'px';
  upsert('checklistByType',{type:'bar',data:{labels:types,datasets},options:{indexAxis:'y',plugins:{legend:{position:'bottom'},tooltip:{callbacks:{label:c=>c.dataset.label+': '+c.parsed.x.toFixed(1)+'%'}}},scales:{x:{beginAtZero:true,max:100,grid:{color:gridColor()},ticks:{callback:v=>v+'%'},title:{display:true,text:'Checklist completion'}},y:{grid:{display:false},ticks:{autoSkip:false}}}}});
}
